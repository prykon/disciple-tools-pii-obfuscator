<?php
/**
 * Plugin Name: Disciple.Tools - PII Obfuscator
 * Plugin URI: https://github.com/DiscipleTools/disciple-tools-pii-obfuscator
 * Description: Disciple.Tools - PII Obfuscator is intended to help developers and integrator jumpstart their extension of the Disciple.Tools system.
 * Text Domain: disciple-tools-pii-obfuscator
 * Domain Path: /languages
 * Version:  0.1
 * Author URI: https://github.com/DiscipleTools
 * GitHub Plugin URI: https://github.com/DiscipleTools/disciple-tools-pii-obfuscator
 ...
 * 1. @todo Rename the `disciple-tools-pii-obfuscator.php file.
 * 2. @todo Refactor all occurrences of the name Disciple_Tools_PII_Obfuscator, disciple_tools_pii_obfuscator, disciple-tools-pii-obfuscator, pii_obfuscator, and "PII Obfuscator"
...
/**
 * Gets the instance of the `Disciple_Tools_PII_Obfuscator` class.
 *
 * @since  0.1
 * @access public
 * @return object|bool
 */
function disciple_tools_pii_obfuscator() {
    $disciple_tools_pii_obfuscator_required_dt_theme_version = '1.19';
    ...
    if ( $is_theme_dt && version_compare( $version, $disciple_tools_pii_obfuscator_required_dt_theme_version, "<" ) ) {
        add_action( 'admin_notices', 'disciple_tools_pii_obfuscator_hook_admin_notice' );
    ...
    return Disciple_Tools_PII_Obfuscator::instance();

}
add_action( 'after_setup_theme', 'disciple_tools_pii_obfuscator', 20 );
 ...
 * @access public
 */
class Disciple_Tools_PII_Obfuscator {
    ...
    private function __construct() {
         ...
         */
        if ( $is_rest && strpos( dt_get_url_path(), 'disciple-tools-pii-obfuscator' ) !== false ) {
         ...
         */
        if ( strpos( dt_get_url_path(), 'metrics' ) !== false || ( $is_rest && strpos( dt_get_url_path(), 'disciple-tools-pii-obfuscator-metrics' ) !== false ) ){
...
//        require_once( 'magic-link/magic-link-home.php' );
    ...
    public static function deactivation() {
        ...
        delete_option( 'dismissed-disciple-tools-pii-obfuscator' );
    }
    ...
    public function i18n() {
        $domain = 'disciple-tools-pii-obfuscator';
    ...
    public function __toString() {
        return 'disciple-tools-pii-obfuscator';
    }
    ...
    public function __call( $method = '', $args = array() ) {
        _doing_it_wrong( "disciple_tools_pii_obfuscator::" . esc_html( $method ), 'Method does not exist.', '0.1' );
        unset( $method, $args );
        return null;
    }
}
...
if ( ! function_exists( 'disciple_tools_pii_obfuscator_hook_admin_notice' ) ) {
    function disciple_tools_pii_obfuscator_hook_admin_notice() {
        global $disciple_tools_pii_obfuscator_required_dt_theme_version;
        $wp_theme = wp_get_theme();
        $current_version = $wp_theme->version;
        $message = "'Disciple.Tools - PII Obfuscator' plugin requires 'Disciple.Tools' theme to work. Please activate 'Disciple.Tools' theme or make sure it is latest version.";
        if ( $wp_theme->get_template() === "disciple-tools-theme" ){
            $message .= ' ' . sprintf( esc_html( 'Current Disciple.Tools version: %1$s, required version: %2$s' ), esc_html( $current_version ), esc_html( $disciple_tools_pii_obfuscator_required_dt_theme_version ) );
        }
        // Check if it's been dismissed...
        if ( ! get_option( 'dismissed-disciple-tools-pii-obfuscator', false ) ) { ?>
            <div class="notice notice-error notice-disciple-tools-pii-obfuscator is-dismissible" data-notice="disciple-tools-pii-obfuscator">
                <p><?php echo esc_html( $message );?></p>
            </div>
            <script>
                jQuery(function($) {
                    $( document ).on( 'click', '.notice-disciple-tools-pii-obfuscator .notice-dismiss', function () {
                        $.ajax( ajaxurl, {
                            type: 'POST',
                            data: {
                                action: 'dismissed_notice_handler',
                                type: 'disciple-tools-pii-obfuscator',
 ...
 * @link https://github.com/DiscipleTools/disciple-tools-pii-obfuscator/wiki/Configuring-Remote-Updating-System
 *
...
//                'https://raw.githubusercontent.com/DiscipleTools/disciple-tools-pii-obfuscator/master/version-control.json',
//                __FILE__,
